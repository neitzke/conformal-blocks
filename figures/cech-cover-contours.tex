\begin{figure}[h]
  ...
  \begin{tikzpicture}[scale=1.0, line cap=round, line join=round]
    ...
    % interfaces c_{alpha beta} between the ordered regions R_alpha and R_beta
    \begin{scope}
      \clip (B) circle (\R);
      \draw[very thick, black] (A) circle (\rV);
      % orientation induced by boundary of R_\alpha (counterclockwise here)
      \draw[black, -{Latex[length=2.2mm,width=1.6mm]}]
        ($(A)+({\rV*cos(35)},{\rV*sin(35)})$) arc[start angle=35,end angle=55,radius=\rV];
    \end{scope}
    \node[black] at (0.10,1.75) {$c_{\alpha\beta}$};

    \begin{scope}
      \clip (C) circle (\R);
      \begin{scope}[even odd rule]
        \clip (-2.8,-2.8) rectangle (2.8,2.8) (B) circle (\R);
        \draw[very thick, black] (A) circle (\rV);
        % orientation induced by boundary of R_\alpha
        \draw[black, -{Latex[length=2.2mm,width=1.6mm]}]
          ($(A)+({\rV*cos(-70)},{\rV*sin(-70)})$) arc[start angle=-70,end angle=-50,radius=\rV];
      \end{scope}
    \end{scope}
    \node[black] at (-1.45,-0.35) {$c_{\alpha\gamma}$};

    \begin{scope}
      \clip (C) circle (\R);
      \begin{scope}[even odd rule]
        \clip (-2.8,-2.8) rectangle (2.8,2.8) (A) circle (\rV);
        \draw[very thick, black] (B) circle (\rV);
        % orientation induced by boundary of R_\beta
        \draw[black, -{Latex[length=2.2mm,width=1.6mm]}]
          ($(B)+({\rV*cos(-120)},{\rV*sin(-120)})$) arc[start angle=-120,end angle=-80,radius=\rV];
      \end{scope}
    \end{scope}
    \node[black] at (1.75,-0.35) {$c_{\beta\gamma}$};
    ...
    \begin{scope}[xshift=5.2cm]
      ...
      % contour c_{12}: interface between the two ordered regions
      \begin{scope}
        \clip (B2) circle (\Rtwo);
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{Cover by patches $U_\alpha$ and shrunken sets. After choosing an ordering on the cover, 
  here $\alpha < \beta < \gamma$,
  the sets $R_\alpha = V_\alpha \setminus \bigcup_{\gamma < \alpha} V_\gamma$ give a partition of the surface, and the contours $c_{\alpha\beta}$ are the interfaces between $R_\alpha$ and $R_\beta$, oriented as boundary components of $R_\alpha$.}
\end{figure}
