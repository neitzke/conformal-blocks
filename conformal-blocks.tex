...
\begin{equation}
Y(a,z) Y(b,w) = Y(Y(a,z-w)b,w) = \sum_{n \in \bbZ} (z-w)^{-n-1} Y(a_{(n)} b, w) \, .  
\end{equation}
This is an equality in $\End \vertex[[z^{\pm 1},w^{\pm 1}]]$, where 
on the right side we mean the expansion in the region $\abs{z} > \abs{w}$.
\end{thm}
...
\end{equation}
It's similar for the other universal enveloping vertex algebras.
...
\end{equation}
What about $Y(J_{(-m)} \vac, z)$?
To calculate this it is useful to know the following:
...
\end{equation}
since the RHS indeed has both of the necessary properties.
...
sometimes called \ti{normal ordered product}:
...
We can also write it in another way which avoids the contour integral:
\begin{multline}
...
Another way to phrase this condition: there is a linear subspace $S \subset W = \otimes_i \vrep_i$ 
