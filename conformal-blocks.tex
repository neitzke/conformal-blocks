...
Here we will be informal: a full proof seems to require some
machinery. But the main idea is like this.
...
$(J_{(-1)} J_{(-1)} \vac)_{(n)} = \sum_{k+\ell = n-1} \nop{J_{(k)} J_{(\ell)}}$, where we defined the \ti{normal ordered product}
\begin{equation}
...
Here is another useful way to think about this object:
\begin{equation}
  \frac{1}{2 \pi \I} \oint_w (z-w)^{-1} J(z) J(w) \, \de z = \frac{1}{2 \pi \I} \oint_w (z-w)^{-1} \left( J(z) J(w) - \frac{1}{(z-w)^2} \right) \, \de z = \lim_{z \to w} \left( J(z) J(w) - \frac{1}{(z-w)^2} \right) \, .
\end{equation}
So $(J_{(-1)} J_{(-1)} \vac)(w)$ can be obtained by starting
with $J(z) J(w)$, deleting ``by hand'' the singular term $\frac{1}{(z-w)^2}$, and taking $z \to w$.
...
\begin{example}
Here is an important class of representations of 
$\vertex = V^k(\fg)$. Fix a $\fg$-module $M$. Then let
...
For instance, suppose $\vrep = \vrep(M)$ and $\vrep' = \vrep(M')$.
...
\fixme{write a proposition here}
