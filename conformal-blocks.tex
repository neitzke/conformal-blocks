...
Then we give the singular part of the expansion $a(z) b(w)$, 
...
\begin{equation}
  \lb{a}{\lambda}{b}(w) = \Res_{z=w} \e^{\lambda(z-w)} a(z) b(w) = \sum_{j \ge 0} \frac{\lambda^j}{j!} a_{(j)} b \, .
\end{equation}
...
sum. The expansion in $\abs{z} > \abs{w}$ involves only negative powers of $z$,
...
\begin{equation}
  Y(\nop{ab}, w) = Y(a,z) Y(b,w) - [Y(a,z)_\ge,Y(b,w)] \vert_{z=w}
\end{equation}
...
Given a chiral correlation system and \ti{any} choice of marked points with local coordinate systems,
we found a corresponding functional $\Phi: \otimes_i \vertex \to \bbC$. Now we can ask: what is the relation
between these functionals for different choices of marked points and coordinates?
...
To describe this action it is enough to give its infinitesimal version, i.e.
we consider the Lie algebra $\fG$ of vector fields on the formal disc which
...
\begin{prop}
Fix a chiral correlation system on a marked curve $C$.
For any choice of local coordinates $z_i$ at the marked points $p_i$, let
\[
\Phi_{(z_i)} : \bigotimes_{i=1}^n \vertex \to \bbC
\]
be the functional obtained from the correlation system by expanding each insertion in the chosen
local coordinate.
If we replace the local coordinate $z_i$ by
\[
\tilde z_i = g_i(z_i)\qquad (i=1,\dots,n),
\]
then the corresponding functional is given by
\[
\Phi_{(\tilde z_i)}
...
Note that \eqref{eq:omega-lambda-bracket} is the $\lambda$-bracket of the generator of the 
...
so both indeed integrate to $\cG$.

\begin{prop} Suppose $\vrep$ is a $\vertex$-module, for which the induced $\fG$-action integrates to $\cG$.
In this case, \fixme{...}
\end{prop}
