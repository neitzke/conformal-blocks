...
Property (5) is sometimes called the ``locality'' axiom: it 
...
\begin{equation}
Y(a,z) Y(b,w) = Y(Y(a,z-w)b,w) = \sum_{n \ge 0} (z-w)^{-n-1} Y(a_{(n)} b, w) \, .  
\end{equation}
This is an equality in $\bbC[[z,w]]$, where 
...
Then we could write 
\begin{align}
a_{(m)} b_{(n)} \chi - b_{(n)} a_{(m)} \chi &= - \frac{1}{4 \pi^2} \left[  \oint_{\abs{z}=1} \oint_{\abs{w} = 1-\eps} - \oint_{\abs{z}=1} \oint_{\abs{w} = 1+\eps} \right]z^m w^n \, F(z,w) \, \de z \de w \\
&= - \frac{1}{4 \pi^2} \left[  \oint_{\abs{z}=1} \oint_{\abs{w-z} = \eps} \right]z^m w^n \, F(z,w) \, \de z \de w \\
&= - \frac{1}{4 \pi^2} \left[  \oint_{\abs{z}=1} z^m \oint_{\abs{w-z} = \eps} \right] w^n \, F(z,w) \, \de z \de w
\end{align}
Now, the polar part of $F(z,w)$ is determined by
\autoref{thm:vertex-algebra-associativity}. \fixme{...}
