...
\begin{example}[Parabolic Verma modules]
...
\begin{example}[Irregular modules]
...
For instance, suppose $\vrep = \Ind^{\Lie(\vertex)}_{\Lie(\vertex)_{\ge 0}} M$ and $\vrep' = \Ind^{\Lie(\vertex)}_{\Lie(\vertex)_{\ge 0}} M'$.
...
Then there is an action of $\cG$ on $\vertex$, and (we will see) 
...
local coordinate.
Let $g_i \in \cG$ be formal changes of coordinate and write $\rho(g_i)\in \End(\vertex)$ for the induced
action of $\cG$ on $\vertex$ constructed above. If we replace the local coordinate $z_i$ by
\[
...
Note that \eqref{eq:omega-lambda-bracket} is the $\lambda$-bracket of the Virasoro algebra.
It implies that if we write
\begin{equation}
...
(It also implies that there is a homomorphism of vertex algebras, $\Vir^c \to V^k(\fg)$.)
