...
\end{equation}
so they form a copy of $\fg$ inside the mode algebra.
...
\begin{prop}[PBW basis for universal vertex algebra]
...
First, we have $\Phi(\vac, \vac) = \IP{1} = 1$.
Then, by contour-deformation we have\footnote{The minus sign here could be confusing if you try to look things up elsewhere:
many authors consider the ``Shopavalov form'' 
$\Phi_S$ on $\vertex$, which is slightly different.
With respect to $\Phi_S$ the adjoint of $J^X_{(n)}$ is $J^{\omega(X)}_{(-n)}$, where $\omega$ is an anti-involution of $\fg$. The two are related by
...
series. (So this gives an alternate way of computing
