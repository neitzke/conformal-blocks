...
A \emph{vertex algebra} is a quadruple \((\vertex, \vac, \partial, Y)\) where
...
\begin{enumerate}
\item The \ti{Heisenberg algebra} $\vertex = \Heis$. It is generated by one defect, i.e. $R$ is
a free $\bbC[\partial]$-module on one generator, which we call $J$. 
The operator product is
\begin{equation}
 J(z) J(w) = \frac{\vac(w)}{(z-w)^2} + \regular
\end{equation}
i.e. $[J_\lambda J] = \lambda \vac$.
% i.e. the only nonzero product is $J_{(1)} J = \bid$.
The mode algebra is spanned by $J_{(n)}$ $(n \in \bbZ)$ and $\bid = \vac_{(-1)}$, with
\begin{equation}
...
It is generated by defects $J_X$ for every $X \in \fg$, with operator products of the form
\begin{equation} \label{eq:kac-moody-ope}
  J^X(z) J^Y(w) = \frac{k \kappa(X,Y) \vac(w)}{(z-w)^2} + \frac{J^{[X,Y]}(w)}{z-w} + \regular
\end{equation}
i.e. $[J^X\,\!_\lambda J^Y] = \lambda k \kappa(X,Y) \vac + J^{[X,Y]}$.
...
\begin{equation} \label{eq:virasoro-ope}
  T(z) T(w) = \frac{\frac{c}{2} \vac(w)}{(z-w)^4} + \frac{2 T(w)}{(z-w)^2} + \frac{\partial T(w)}{z-w} + \regular
\end{equation}
i.e. $[T_\lambda T] = \lambda^3 \frac{c}{12} \vac + (2 \lambda + \partial) T$.
...
When $\vertex = U(R)$, we have a PBW-type basis:

\begin{prop}[PBW basis for universal vertex algebra]
Let $R$ be a Lie conformal algebra which is free as a $\bbC[\partial]$-module, and fix an
ordered $\bbC[\partial]$-basis $(a^i)_{i\in I}$ of $R$.
Then the vectors
\[
...
Equivalently, if $\mathfrak L$ is the mode Lie algebra on $R$, and
