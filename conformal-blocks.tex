...
\end{equation}
i.e. $[J_\lambda J] = \lambda \bid$, while $\bid$ is central.
...
and we recall the dual Coxeter number $h^\vee$ from \fixme{...}
The key fact about $\omega$ is:
\begin{prop} The $\lambda$-bracket of $\omega$ with itself and with $J^X$ are
\begin{equation}
[\omega_\lambda \omega] = \frac{c}{12} \lambda^3 \bid + (2 \lambda + \partial) \omega  , \qquad [\omega_\lambda J^X] = (\partial + \lambda) J^X \, .
\end{equation}
with
\begin{equation}
...
\end{equation}
\end{prop}
One can prove this by repeated use of the ``noncommutative Wick formula'' which reads
\begin{equation}
  [a\,_\lambda\,\nop{bc}] = \nop{(a\,_\lambda\,b) c} + \nop{b (a\,_\lambda\,c)} + \int_0^\lambda [[a_\mu b]_{\lambda - \mu} c]
\end{equation}
...
\end{equation}
which is the Virasoro Lie algebra. Note that if we just look at the modes with $n \ge 0$
they give \eqref{eq:witt-algebra}.
Thus this is a second way of getting an action of the Witt algebra on $\vertex$.
Fortunately it actually coincides with the first way.
