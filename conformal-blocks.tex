...
$\vertex$, associated to a small disc around $\vertex$).
...
Some remarks:
\begin{itemize}
\item Property (5) is sometimes called the ``locality'' axiom: it 
...
the two vectors
$J_{-1}^4 \vac$, $J_{-2}^2 \vac$, $J_{-3} J_{(-1)} \vac$, $J_{-4} \vac$.
\end{example}
...
happens at
$k = 1, 2, -2, -\frac34$.
The general answer is that $\Phi$ is degenerate iff
$k = -2 + p/q$ for some $p \ge 2$, $q > 0$ (in particular,
this happens if $k \in \bbZ_{\ge 0}$.)
...
under all $Y(a,z)$), and we can form a quotient $\vertex/I$, 
which is still a vertex algebra.
...
\begin{defn}
A chiral correlation system for $V^k(\fg)$ on $C$ is
...
Note that in contrast to the case before, here 
having chiral correlation
...
$\Phi: \prod_{i=1}^n \vrep_i \to \bbC$, and we also have
the 1-form $\IP{J(z) \prod_{i=1}^n \chi_i(p_i)}$. This 1-form is meromorphic, with singularities only at the $p_i$,
so the sum of its residues must be zero. That gives the condition
\begin{equation}
...
\subsection{Changes of coordinate}
