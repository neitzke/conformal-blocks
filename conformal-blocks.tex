...
functions on $\bbC \bbP^1$. Then these elements 
form an ideal $I$ in $\vertex$ (meaning $I$ is stable
under all $Y(a,z)$), in fact the maximal ideal (radical). 
Then we can form a quotient $L_k(\fg) = \vertex/I$, 
which is a simple vertex algebra.

Complete formulas for these determinants are given in
\fixme{Kac-Kazhdan}.
...
Let $\hat\fg$ be the Lie algebra generated by the modes $J^X_{(0)}$.
Then suppose $\vrep$ is a $\hat\fg$-module 
...
Then let 
$\vrep = \Ind^{\hfg}_{\hfg_{\ge 0}} M$.\footnote{Recall the definition: $\Ind_\fh^\fg M = U(\fg) \otimes_{U(\fh)} M$.}
...
Another interesting example: suppose $\vrep = \Ind^{\Lie(\vertex)}_{\Lie(\vertex)_{\ge 0}} M$ as above, but
$\vrep'$ is an irregular module as in \autoref{exa:irregular-module}. In this case by a similar analysis
we conclude that the space of chiral correlation systems is isomorphic to $M^*$.
...
where $\vrep_i = \Ind^{\Lie(\vertex)}_{\Lie(\vertex)_{\ge 0}} M_i$.
...
\begin{prop} Fix $C$ with markings as above.
...
the correlation functions are compatible with this action:
\begin{prop}
...
\begin{prop} The $\lambda$-bracket of $\omega$ with itself and with $J^X$ are
...
To get an idea of the proof of the lemma we can use successive contour-integration:
indeed the left side could be written as \fixme{make figure with contours}
\begin{equation}
  \oint \oint \de z \, \de w \, a(z) b(w) c(0) \frac{\e^{\lambda z}}{w} 
\end{equation}
while the first two terms on the right side are of the shape
\begin{equation}
  \oint \oint \de z \, \de w \, a(z) b(w) c(0) \frac{\e^{\lambda (z-w)}}{w}, \qquad  \oint \oint \de z \, \de w \, a(z) b(w) c(0) \frac{\e^{\lambda z}}{w}
\end{equation}
...
we get an extra bonus: it acts not only on $\vertex$ but on all $\vertex$-modules $\vrep$. Thus we can try
to use it to make a coordinate-independent version of our definition of chiral correlation systems.
For this we need to integrate from $\fG$ to the group $\cG$ of coordinate changes.
Whether we can really do it depends on $\vrep$.
...
We have:
\begin{prop} Assume $k \neq h^\vee$. Suppose $\vrep_i$ are $\vertex$-modules, for which the induced $\fG$-action integrates to $\cG$.
