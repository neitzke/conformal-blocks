...
We begin with a basic example of a (Euclidean, 2-dimensional) \ti{conformal field theory}:
the (uncompactified) \ti{free scalar field}.
Take $C$ to be a compact Riemannian 2-manifold (maybe not orientable).
In this field theory the basic object is a map $\phi: C \to \bbR$
and our interest is in computing integrals over the space $\cF$ of all such maps,
for example
\begin{equation}
  Z = \IP{1} = \int_{\phi \in \cF} \e^{-S(\phi)}
\end{equation}
where the \ti{action} is
\begin{equation}
  S(\phi) = \int_C \norm{\de \phi}^2 \, \de \vol_C \, .
\end{equation}
An important observation is that the action is \ti{conformally invariant},
in the sense that if we rescale the metric on $C$ by
\begin{equation}
  g_C \to \e^{f} g_C
\end{equation}
we get $\vol_C \to \e^{f} \vol_C$ and $\norm{\de \phi}^2 \to \e^{-f} \norm{\de \phi}^2$,
so that $S(\phi) \to S(\phi)$.
