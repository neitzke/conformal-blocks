...
First you show using (2)-(4) that
\begin{equation}
  Y(a,z) Y(b,w) \vac = Y(Y(a,z-w) b, w) \vac
\end{equation}
(You use the integrated form of (4), namely 
$\e^{z T} Y(a,w) \e^{-z T} = Y(a,z+w)$, and combining
that with (3), $\e^{z T} b = Y(b,z) \vac$.)
...
allowed using axiom (5), and then set $u = 0$ and divide
...
action of the ``zero modes'' $J^X_0$.
...
\subsection{Including marked points}
...
Next we want to upgrade all this again to include modules.
