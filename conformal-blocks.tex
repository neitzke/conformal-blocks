...
For instance, suppose $\vrep = \Ind^{\Lie(\vertex)}_{\Lie(\vertex)_{\ge 0}} M$ as in 
...
\begin{prop}[Conformal blocks vs. chiral correlation systems] Fix compact connected $C$ 
with markings as above. By Fourier expansion we obtain a linear map from the space of chiral correlation systems
for $V^k(\fg)$ on $C$ to the space of conformal blocks. This map is injective as long as we have at least
one marked point.
\end{prop}
...
\fixme{discuss case of $\cG_+$ instead of $\cG$}

\fixme{discuss ``local'' version of $\vertex$, $\vrep$}
