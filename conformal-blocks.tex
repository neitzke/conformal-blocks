...
The basic data of the course are:

\begin{enumerate}
\item A \ti{vertex algebra} $\vertex$. This means a vector space $\vertex$ of ``states'' or ``(local) defects.'' We think of the 
states $\psi \in \vertex$ as objects which can be inserted at a point in the complex plane, giving an object $\psi(z)$.
Moreover, when we have $\psi, \phi \in \vertex$ we can make a ``product'' $\psi(z) \phi(w)$,
getting a meromorphic expansion of the form 
\begin{equation}
 \psi(z) \phi(w) = \sum_{n} (z-w)^{-n-1} (\psi_{(n)} \phi)(w) 
\end{equation}
where each $\psi_{(n)} \phi \in \vertex$.
This operation is supposed to be associative in a precise sense
(which amounts to some complicated constraints on the products $(a,b) \mapsto a_{(n)} b$).

Here are three main examples: 
\begin{enumerate}
\item The \ti{Heisenberg algebra} $\vertex = \Heis$. It is freely generated by one defect $J$ with an expansion of the form
\begin{equation}
 J(z) J(w) = \frac{\bid(w)}{(z-w)^2} + \cdots
\end{equation}
\item For any simple complex Lie algebra $\fg$ and $k \in \bbC$ one has the \ti{affine Kac-Moody algebra} $\vertex = V^k(\fg)$, e.g. $V^k(\fsl_2)$.
It is freely generated by defects $J_X$ for every $X \in \fg$, with an expansion of the form
\begin{equation}
  J_X(z) J_Y(w) = \frac{k \IP{X,Y} \bid(w)}{(z-w)^2} + \frac{J_{[X,Y]}(w)}{z-w} + \cdots
\end{equation}
where $\IP{\cdot,\cdot}$ is the Killing form.
\item For any $c \in \bbC$ one has the \ti{Virasoro algebra} $\Vir^c$. It is freely generated by a defect $T$, with an expansion of the form
\begin{equation}
  T(z) T(w) = \frac{\frac{c}{2} \bid(w)}{(z-w)^4} + \frac{2 T(w)}{(z-w)^2} + \frac{\partial_w T(w)}{z-w} + \cdots
\end{equation}
\end{enumerate}

We also have a notion of a \ti{representation} of $\vertex$: this means another vector space $\vrep$, such that given
$\psi \in \vertex$ and $\chi \in \vrep$ we also have a meromorphic expansion $\psi(z) \chi(w) = \sum_n (z-w)^n (\psi_{(n)} \chi)(w)$,
with each $\psi_{(n)} \chi \in \vrep$. Again the operation is supposed to be associative.\footnote{It seems like $\vertex$ and $\vrep$ play
similar roles, so you might wonder why not combine them into a bigger vertex algebra. The reason is that we haven't specified anything
about what happens when we bring two elements of $\vrep$ close together. In many cases there is a sensible way
to formulate and answer this question, but the answer might not be a meromorphic product of the sort we need for a vertex algebra.}

We give a full definition in \fixme{appendix}.

We always consider vertex algebras $\vertex$ which are \ti{conformal}, meaning they contain a copy of $\Vir^{c}$ for some $c$.

\item A \ti{Riemann surface} $C$, usually compact, perhaps with marked points, each marked point labeled by a representation of $\vertex$
(and maybe some additional tangent-space data, like a local coordinate).
...
\begin{equation}
  \Conf(C, \vertex)
\end{equation}
The spaces $\Conf(C, \vertex)$ are the main object of study in this course.
Before defining them I want to try to convince you they are worth learning about.
Here are some of their uses.


\subsubsection{Invariant spaces}

Suppose $\vertex = V^k(\fg)$, with Cartan $\fh \subset \fg$. 
Then for each dominant integral weight $\lambda \in \fh^*$ 
there is a corresponding finite-dimensional representation $R_\lambda$ of $\fg$,
and also a corresponding infinite-dimensional representation $M_\lambda$ of $V^k(\fg)$.

Now take $C = \bbC\bbP^1$, with marked points $z_1, \dots, z_n$, carrying representations
$M_{\lambda_1}, \dots, M_{\lambda_n}$. Then one has\footnote{The isomorphism we get here depends on $z_1 ,\dots, z_n$;
we'll see this from another point of view momentarily. It alos depends on $k$.}
\begin{equation}
\Conf(C, \vertex) = \Hom(R_{\lambda_1} \otimes \cdots \otimes R_{\lambda_n}, R_0).
\end{equation}
With 3 punctures this is equivalent to the tensor product multiplicity spaces, i.e.
\begin{equation}
\Conf(C, \vertex) = \Hom(R_{\lambda_1} \otimes R_{\lambda_2}, R_{\lambda_3}^*).
\end{equation}
But $\Conf(C,\vertex)$ has lots of extra structures, so we get some new
structure in representation theory this way.


\subsubsection{Connections and braiding}

We can consider varying the base curve $C$, and then the spaces $\Conf(C, \vertex)$
are the fibers of a twisted $D$-module $\Conf(\cdot, \vertex)$ over some moduli space $\cM$ of curves.
...
For instance, suppose $C$ is $\bbC\bbP^1$ with $n$ 
...
An example of a strongly rational vertex algebra
is: consider $V^k(\fg)$; it is simple for $k \notin \bbZ_+$; for $k \in \bbZ_+$ it is not simple 
but has a simple quotient $L_k(\fg)$, and $L_k(\fg)$ is strongly rational.

If $\vertex$ is strongly rational, in particular it has only finitely many irreducible representations, 
and moreover the spaces $\Conf(C, \vertex)$ are finite-dimensional, for any compact $C$, with any
irreducible representations inserted at punctures. So this is a nice world where we do not have to worry
about functional analysis, ind- or pro-objects, etc: everything will be about finite-dimensional spaces.

In this case
the representations $\vrep$ are objects of a \ti{modular tensor category}.
It means in particular that you have, in addition to the braiding, ``cup'' and ``cap''
operations which create/destroy a pair of marked points.
Then you can use them to build framed link invariants: indeed, letting $C$ be
...
\subsubsection{Holomorphic factorization} In a 2-dimensional conformal field theory
...
\begin{equation}
  \DIP{J(z_1) \cdots J(z_n) \bJ(z_1) \cdots \bJ(z_m)} 
\end{equation}
where classically $J = \partial \Phi$ and $\bJ = \overline\partial \Phi$.
Then one finds an approximate factorization
\begin{equation}
  \DIP{J(z_1) \cdots J(z_n) \bJ(z_1) \cdots \bJ(z_m)} = \int \de a \, \IP{J(z_1) \cdots J(z_n)}_{\Psi_a} \IP{\bJ(z_1) \cdots \bJ(z_m)}_{\overline\Psi_a}
\end{equation}
...
numerator. Or, slightly more abstractly: the
determinant $1 / \sqrt{\det \Delta}$ should most naturally be
understood as providing a measure on the space of zero modes,
which is $\frac{\de \Phi_0}{\sqrt{\det \Delta'}} = \frac{\sqrt{A}}{\sqrt{\det \Delta'}} \mu_{L^2}$.}
...
Then we have
\begin{equation}
  \DIP{1} = \int_{a \in \I \bbR} \frac{\de a}{2 \pi \I} \, \abs{\IP{1}_{\Psi_a}}^2  = \frac{1}{\sqrt{2 \im \tau} \abs{\eta(\tau)}^2}
\end{equation}

\subsubsection{Tau functions}
...
placed at points $(0,1,\infty,q)$. At each puncture we place a simple module for $\Vir_{c=1}$
(a ``Verma module'').
...
\subsubsection{The AGT correspondence} \fixme{...}

\subsubsection{Quantum groups} \fixme{...}

\subsubsection{Algebraic geometry} \fixme{...}
...
\begin{equation} \label{eq:gaussian-det}
  \IP{1} = \int_{\cF} \de \vec{x} \, \e^{-\half x^i M_{ij} x^j} = \frac{(2\pi)^{N/2}}{\sqrt{\det M}}.
\end{equation}

\begin{exercise}
Prove that $\IP{x^{i_1} \cdots x^{i_k}} = 0$ whenever $k$ is odd,
and compute directly 
\begin{equation}
\frac{\IP{x^i x^j}}{\IP{1}} = (M^{-1})^{ij} \, .  
\end{equation}
\end{exercise}
...
\begin{equation}
  \IP{T_{ij}(x)} = \frac{2}{\sqrt{\det g}} \frac{\delta}{\delta g^{ij}(x)} \IP{1} \, .
\end{equation}
We use this condition in its logarithmic form
\begin{equation}
  \frac{\IP{T_{ij}(x)}}{\IP{1}} = \frac{2}{\sqrt{\det g}} \frac{\delta}{\delta g^{ij}(x)} \log \IP{1}
\end{equation}
\fixme{...}
...
\begin{equation}
  \IP{J(z_1) \cdots J(z_k) \bJ(w_1) \cdots \bJ(w_l)}
\end{equation}
...
\subsection{The zeta-regularized determinant}

One expects that
\begin{equation}
  \IP{1} = 1 / \sqrt{\det{}' \Delta}
\end{equation}
...
To formalize things,
we introduce a symbol ``$Y(\psi,z)$'' which should be understood to mean ``the power series expansion 
of $\psi(z)$ around $z = 0$.''
...
So the symbol I wrote $\psi(z) \phi(w)$ will get represented by \ti{two} different things in this 
formalization.
But they are closely related: they are $Y(\psi,z) Y(\phi,w)$ and $Y(\phi, w) Y(\psi,z)$.

A \emph{vertex algebra} is a quadruple \((\vertex, Y, \mathbf{1}, T)\) where
\(\vertex\) is a (complex) vector space, \(\bid \in \vertex\),
...
\end{equation}
is a linear map, such that for all $a, b \in \vertex$

\begin{enumerate}

\item For each \(a,b\) we have \(a_{(n)}b=0\) for all \(n\gg 0\).
...
Motivation of "conformal block" from the point of view of free boson?
