...
$\vertex = V^k(\fg)$. Fix a $\fg$-module $M$. Let
$\Lie(\vertex)_{\ge 0}$ act on $M$ as follows:
the zero modes act through the $\fg$-action on $M$,
and the positive modes act as zero.
Then let 
$\vrep = \Ind^{\Lie(\vertex)}_{\Lie(\vertex)_{\ge 0}} M$.
As a vector space $\vrep$ is freely generated 
from $M$ by action of the $J^X_{(n)}$ with $n < 0$.
...
Then let $\Lie(\vertex)_{\ge 1}$ act on $\bbC$
as follows: the modes $J^X_{(1)}$ act by $u(X)$,
all higher modes $J^X_{(n)}$ act by $0$.
Then let $\vrep = \Ind^{\Lie(\vertex)}_{\Lie(\vertex)_{\ge 1}} \bbC$.
\end{example}
...
$\vrep'$ is an irregular module as in \autoref{exa:irregular-module}. In this case \fixme{...}
The conclusion is that the space of chiral correlation systems is isomorphic to $M^*$.
