...
% We give a full definition in \autoref{sec:vertex-algebras};
% here we will be more informal.
...
Usually we consider
\ti{ordinary $\vertex$-modules}, which means that \eqref{eq:y-series} is a Laurent series,
i.e. $a^\vrep_{(n)} \chi = 0$ for $n \gg 0$, i.e. $Y^\vrep(a,z) \chi$ is formally meromorphic in $z$. In particular, we require
that $\vertex$ itself is an ordinary $\vertex$-module.
...
\begin{exercise}
Prove that the modes of $\vertex$ form a Lie algebra, with
...
The vertex algebras we consider are of a simple sort:
...
\begin{enumerate}
\item The \ti{Heisenberg algebra} $\vertex = \Heis$. It is generated by one defect $J$ with an expansion of the form
\begin{equation}
 J(z) J(w) = \frac{\bid(w)}{(z-w)^2} + \cdots
\end{equation}
i.e. the only nonzero product is $J_{(1)} J = \bid$.
The corresponding mode algebra is spanned by $J_{(n)}$ $(n \in \bbZ)$ and $\bid = \bid_{(-1)}$, with
\begin{equation}
[J_{(m)},J_{(n)}] = m \delta_{m+n,0} \bid \, .
\end{equation}

\item For any complex Lie algebra $\fg$ with invariant bilinear form $\IP{\cdot,\cdot}$ and $k \in \bbC$, 
one has the \ti{affine Kac-Moody algebra} $\vertex = V^k(\fg)$.
It is generated by defects $J_X$ for every $X \in \fg$, with an expansion of the form
\begin{equation} \label{eq:kac-moody-ope}
  J^X(z) J^Y(w) = \frac{k \IP{X,Y} \bid(w)}{(z-w)^2} + \frac{J^{[X,Y]}(w)}{z-w} + \cdots
\end{equation}
i.e. $J^X_{(1)} J^Y = k \IP{X,Y}$ and $J^X_{(0)} J^Y = J^{[X,Y]}$.
The corresponding mode algebra is
\begin{equation}
[J^X_{(m)},J^Y_{(n)}] = J^{[X,Y]}_{(m+n)} + m k \IP{X,Y} \delta_{m+n,0} \bid \, .
\end{equation}
...
[L_{(m)},L_{(n)}]
=(m-n)\,L_{(m+n-1)}+\frac{c}{12}\,m(m-1)(m-2)\,\delta_{m+n,2} \, .
\end{equation}
\end{enumerate}
...
% We also have a notion of a \ti{representation} of $\vertex$:\footnote{To be more precise I think I mean \ti{ordinary module}.} this means another vector space $\vrep$, such that given
% $\psi \in \vertex$ and $\chi \in \vrep$ we also have a meromorphic expansion $\psi(z) \chi(w) = \sum_n (z-w)^{-n-1} (\psi_{(n)} \chi)(w)$,
...
I think that these are actually the more important objects.

\begin{defn}[Chiral correlation system]
Suppose given a Riemann surface $C$, with $n \ge 0$ marked
points $p_i$ and $\vertex$-modules $\vrep_i$, with fixed local
coordinate $z_i$ around each $p_i$.
A \ti{chiral correlation system} for $V^k(\fg)$
on $C$ is a rule that assigns an element
\begin{equation}
  \bigg\langle \prod_{j=1}^m J^{X_j}(q_j) \prod_{i=1}^n \chi_i(p_i) \bigg\rangle \in \bigotimes_{j=1}^m T^*_{q_j} C  
\end{equation}
for every $m \ge 0$, $X_j \in \fg$, $q_j \in C$, $\chi_i \in \vrep_i$,
which:
\begin{itemize}
  \item is linear in each $X_i$ and in each $\chi_j$,
  \item is symmetric under the interchange of the $J^{X_i}(q_i)$,
  \item is holomorphic in the $q_i$, away from
  loci where some $q_i = q_j$ or some $q_j = p_i$,
...
\begin{example}[Chiral correlation systems for the Heisenberg vertex algebra]
\fixme{...}
\end{example}
