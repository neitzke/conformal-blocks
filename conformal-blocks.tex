...
Y(a,z) = \sum_{n\in \mathbb{Z}} a^\vrep_{(n)} z^{-n-1},
\end{equation}
such that
\begin{enumerate}

\item For each \(a \in \vertex \), \( \chi \in \vrep \) we have \(a_{(n)}\chi=0\) for all \(n\gg 0\) \, ,
...
\begin{example}[Chiral correlation systems for an affine Kac-Moody algebra on the sphere]
\fixme{...}
\end{example}
